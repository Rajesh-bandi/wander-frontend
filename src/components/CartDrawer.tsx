...
export function CartDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { cart, getProduct, setCartQty, removeFromCart, clearCart } = useStore();
          ...
          >
                    ...
                    <li key={p.id} className="flex gap-3 rounded-2xl border border-border p-3">
                      ...
                      <div className="flex flex-1 flex-col">
                        ...
                        <div className="mt-auto flex items-center justify-between">
                          ...
                          <span className="text-sm font-semibold">${(p.price * c.qty).toFixed(2)}</span>
                        </div>
                      </div>
                    </li>
                  ))}
            ...
            {items.length > 0 && (
              <div className="space-y-3 border-t border-border bg-card/50 p-5">
                ...
                <button
                  onClick={() => { clearCart(); onClose(); toast.success("Order placed", { description: "Your travel gear is on its way." }); }}
                  className="w-full rounded-full bg-primary py-3 text-sm font-semibold text-primary-foreground transition hover:opacity-90 active:scale-[0.98]"
                >
                  Checkout
                </button>
              </div>
